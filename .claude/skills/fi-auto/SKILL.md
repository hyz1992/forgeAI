...
自动执行，实现**架构定义的全部后端模块**和**需求定义的全部前端页面**。

**完整性要求**：fi-implement 必须实现 `docs/architecture.md` 中的每个模块、`docs/requirements.md` 中的每个页面，而不仅限于已有测试的模块。禁止仅实现 auth + 简陋首页后停止。

**输出**：
- `server/src/modules/`
- `client/src/`（如适用）

**完成后**：自动 `git commit -m "chore(fi-auto): fi-implement 完成"`（若有变更）
