...
- 数据流
- API 设计
- **外部服务集成**（当 requirements.md 包含 AI 服务需求时为**必须**）
  - 适配器层设计：每个外部服务对应一个适配器，列出适配器文件名和主要方法签名
  - SDK/API 选择：明确指出使用的 SDK（如 `openai` npm 包）或 REST API 端点
  - 调用模式：同步/异步任务、轮询策略、重试策略
  - 文件存储：生成的图片/音频/视频的本地存储路径和访问方式
  - 引用的全局技能：标注「实现时参照 `dashscope-media` 技能」等
...
- **基础设施**（如适用）
  ...
  - CI/CD 流程
- **前端页面内容规格**（必须，全栈项目）
  - 为每个前端页面生成**页面内容规格**，供 fi-implement 阶段实现具体 UI：
    - 页面名称与路由
    - 主要 UI 区块（如：顶部导航、项目列表、创建按钮、搜索框）
    - 数据来源（调用的 API、使用的 Store）
    - 用户交互（点击、表单提交、导航跳转）
    - 涉及的 API 调用（`GET /api/projects`、`POST /api/projects` 等）
  - 禁止仅列出页面名称和路由；必须描述每个页面应包含什么 UI 元素和交互
- **项目文件结构示例**（必须）
